...
public class MecanumController implements MecanumDrive {
    ...
    public void translate(double x, double y, MecanumDrive.TranslationMethod way) {
        translate(new Angle(x, y), Math.hypot(x, y), way);
    }
    
    public void translate(Angle ang, double speed, MecanumDrive.TranslationMethod way) {
    ...
    public void spinDrive(double x, double y, double turn, MecanumDrive.TranslTurnMethod way) {
    ...
    public void orbit(Point relativeCenter, double power) {
        double xVal = relativeCenter.x;
        double yVal = relativeCenter.y;
        double r = Math.hypot(xVal, yVal);
        Angle ang = new Angle(yVal, -xVal);
        if (getWheelbaseWidth() > r) {
            spinDrive(ang, r / getWheelbaseWidth(), 1, MecanumDrive.TranslTurnMethod.EQUAL_SPEED_RATIOS);
        } else {
            spinDrive(ang, 1, getWheelbaseWidth() / r, MecanumDrive.TranslTurnMethod.EQUAL_SPEED_RATIOS);
        }
    }
